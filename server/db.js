...
function initDb() {
    db.serialize(() => {
        ...
        db.run(`ALTER TABLE users ADD COLUMN last_username_change DATETIME`, (err) => { /* ignore */ });
        ...
        db.run(`ALTER TABLE posts ADD COLUMN goal INTEGER DEFAULT 100`, (err) => { /* ignore */ });
        ...
        db.run(`CREATE TABLE IF NOT EXISTS duels (
            ...
            status TEXT CHECK(status IN ('waiting', 'active', 'p1_won', 'p2_won', 'draw')) DEFAULT 'waiting',
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (post_id) REFERENCES posts(id),
        ...
        db.run(`CREATE TABLE IF NOT EXISTS notifications (
            ...
            post_id INTEGER,
            type TEXT CHECK(type IN ('comment', 'upvote')),
            message TEXT,
